...
**Security validation:** The chart will fail to deploy if `log10xApiKey` is empty or contains the placeholder value `"YOUR-API-KEY-HERE"` when creating a new secret.
...
Config files are mounted at `/data/config/` in the container.
...
When jobs specify `github.config` or `github.symbols`, the chart automatically adds an init container using the [github-config-fetcher](https://github.com/log-10x/log-10x/tree/main/docker-images/ext/github-config-fetcher) image. This init container:
